...
//     can flip the first bit from the right to get 110, flip the second bit from 
//     the right to get 101, flip the fifth bit from the right (a leading zero) 
//     to get 10111, etc.
...
// To get minimum bit flips we find XOR of two number : which have set bits only 
// at those places where A differs from B.
// So, after getting the xor ( a ^ b ) , we need to count the number of set bits.
// We can do that using __builtin_popcount(i): This function is used to count the 
...
// // O(No. of set bits in start^goal)
