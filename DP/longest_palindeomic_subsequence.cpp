...
// O(2^n) Brute force. If the two ends of a string are the same, then 
// they must be included in the longest palindrome subsequence. 
...
// O(n^2) time, O(n) space dp. In #3, the current row is computed 
// from the previous 2 rows only. So we don''t need to keep all the rows.
